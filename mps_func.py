...
def addition_MPS_compression_variational(mps_comp, mps_target_1, mps_target_2, max_iter=30, tol=1e-4,
                                         verbose=0):
    '''
    Variational Compression on MPS with mps_comp given, such that
    |mps_comp > = ( |mps_target_1> + |mps_target_2> ) / N

    Input:
        mps_comp: MPS for optimization
            The input should be in right canonical form BBBBBB and
            it should be normalized.
        mps_target_1: The target to approximate.
            It is not necessary in canonical form and not necessarily
            normalized.
        mps_target_2: The target to approximate.
            It is not necessary in canonical form and not necessarily
            normalized.

    Output:
        trunc_err
        modification mps_comp inplace still in right canonical form
    '''
    L = len(mps_comp)
    # Check normalization
    if np.abs(MPS_dot(mps_comp, mps_comp) - 1.) > 1e-8:
        print(('mps_comp_var not normalized', MPS_dot(mps_comp, mps_comp)))
        raise
    ...
    while (num_iter < max_iter and not conv):
        num_iter += 1
        # Creat cache of environment
        cache_env_list_1 = [None] * L
        cache_env_list_2 = [None] * L
        MPS_dot_right_env(mps_comp,
                          mps_target_1,
                          0,
                          cache_env_list=cache_env_list_1)
        MPS_dot_right_env(mps_comp,
                          mps_target_2,
                          0,
                          cache_env_list=cache_env_list_2)

        # site = 0
        right_env_1 = cache_env_list_1[1]
        right_env_2 = cache_env_list_2[1]
        left_env_1 = np.eye(1)
        left_env_2 = np.eye(1)

        update_tensor_1 = einsum('ij,jkl->ikl', left_env_1, mps_target_1[0])
        update_tensor_1 = einsum('ikl,ml->ikm', update_tensor_1, right_env_1)
        update_tensor_2 = einsum('ij,jkl->ikl', left_env_2, mps_target_2[0])
        update_tensor_2 = einsum('ikl,ml->ikm', update_tensor_2, right_env_2)
        mps_comp[0] = update_tensor_1 + update_tensor_2
        # svd to shift central site
        ...
        for site in range(1, L - 1):
            ...
            left_env_1 = einsum('ijk,ijl->kl', left_env_1, mps_target_1[site])
            cache_env_list_1[site] = left_env_1
            cache_env_list_1[site + 1] = None
            left_env_2 = einsum('ij,ikl->jkl', left_env_2,
                                mps_comp[site].conjugate())
        ...
        left_env_1 = cache_env_list_1[L - 2]
        update_tensor_1 = einsum('ij,jkl->ikl', left_env_1, mps_target_1[L - 1])
        update_tensor_1 = einsum('ikl,ml->ikm', update_tensor_1, right_env_1)
        right_env_2 = np.eye(1)
        left_env_2 = cache_env_list_2[L - 2]
        update_tensor_2 = einsum('ij,jkl->ikl', left_env_2, mps_target_2[L - 1])
        update_tensor_2 = einsum('ikl,ml->ikm', update_tensor_2, right_env_2)
        mps_comp[L - 1] = update_tensor_1 + update_tensor_2
        # svd to shift central site
        ...
        for site in range(L - 2, 0, -1):
            ...
            right_env_1 = einsum('kli, ij ->klj', mps_comp[site].conjugate(),
                                 right_env_1)
            right_env_1 = einsum('klj,mlj->km', right_env_1, mps_target_1[site])
            cache_env_list_1[site] = right_env_1
            cache_env_list_1[site - 1] = None
            right_env_2 = einsum('kli, ij ->klj', mps_comp[site].conjugate(),
                                 right_env_2)
            right_env_2 = einsum('klj,mlj->km', right_env_2, mps_target_2[site])
            cache_env_list_2[site] = right_env_2
            cache_env_list_2[site - 1] = None
